...
export default function Review(props: Props) {
    ...
    async function approveRequest() {
        ...
        if (response.error) {
            ...
            setSuccessMessageList([{ message: 'You have approved the request for carbon tax exemption.' }])
            return
        }
    }
    ...
    async function rejectRequest() {
        ...
        if (response.error) {
            ...
            setSuccessMessageList([{ message: 'You have rejected the request for carbon tax exemption.' }])
            return
        }
    }
    ...
    return (
        <>
            ...
            {errorList.length > 0 && errorList.map((e: any) => (
                <Alert key={e.message} severity="error">
                    {e?.stack ?? e.message}
                </Alert>
            ))}
            {successMessageList.length > 0 && successMessageList.map((e: any) => (
