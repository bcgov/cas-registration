...
=======
Client/app/api/auth/[...nextauth] defines the options object for the keycloak authentication provider, session settings, JWT settings, and callbacks for sign-in, redirect, and session management. See .env.example for option object defaults and see 1Password "" file for option object secrets.
>>>>>>> 2e286ee (📖 chore: README)
...
=======
Since next-auth creates its own token, it doesn't automatically give you access to the original information from the keycloak provider.
>>>>>>> 2e286ee (📖 chore: README)
...
=======
Client\middleware implements middleware chaining to secured the app routes based on NextAuth authentication JWT from keycloak provider.
>>>>>>> 2e286ee (📖 chore: README)
