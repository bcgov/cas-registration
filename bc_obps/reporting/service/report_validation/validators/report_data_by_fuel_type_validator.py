...
def validate_methodology_reporting_fields(
    ...
    for k, v in methodology_record.json_data.items():
        ...
        if not ExpectedValueRangeMethodologyField.objects.filter(
            fuel_type=fuel_record.fuel_type,
            methodology=methodology_record.methodology,
            reporting_field__slug=k,
        ).exists():
            continue
...
class ReportDataByFuelTypeValidator(ReportValidator):
    @classmethod
    def validate(
        cls,
        ...
        for fuel_record in report_fuel_records:
            if not ExpectedValueRangeFuelAmount.objects.filter(fuel_type=fuel_record.fuel_type).exists():
                continue

            activity_name = fuel_record.report_source_type.report_activity.activity.name
            source_type_name = fuel_record.report_source_type.source_type.name
            fuel_type_name = fuel_record.fuel_type.name
            ...
            for methodology_record in ReportMethodology.objects.filter(report_emission__report_fuel_id=fuel_record.id):
                errors.update(
