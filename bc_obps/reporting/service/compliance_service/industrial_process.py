...
class BiogenicEmissionsSplit:
    ...
    lime_recovery_kilm_ratio: Decimal

    def __post_init__(self) -> None:
        if not self.chemical_pulp_ratio + self.lime_recovery_kilm_ratio == Decimal('1'):
...
def compute_industrial_process_emissions(rp: ReportProduct) -> Decimal:
    ...
    if (
        ...
        and rp.product.name in ["Pulp and paper: chemical pulp", "Pulp and paper: lime recovery kiln"]
    ):
        ...
        if rp.product.name == "Pulp and paper: lime recovery kiln":
            return (
                industrial_process
                - overlapping_industrial_process_emissions * biogenic_emissions_split.lime_recovery_kilm_ratio
            )
