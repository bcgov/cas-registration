...
class ReportRlsTest(BaseTestCase):
    ...
    def test_report_rls_industry_user(self):
        ...
        assert report.__class__.objects.count() == 2

        def select_function(cursor):
            # Select the report for the approved user operator and not the random report
            assert Report.objects.filter(reporting_year=reporting_year).count() == 1
        ...
        def delete_function(cursor):
            assert Report.objects.filter(operation=operation).count() == 1
            # Delete the report for the approved user operator
            number_of_deleted_reports, _ = report.delete()
            assert Report.objects.filter(operation=operation).count() == 0
            assert number_of_deleted_reports == 1
