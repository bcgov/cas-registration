...
class ReportAdditionalDataRlsTest(BaseTestCase):
    ...
    def test_report_additional_data_rls_industry_user(self):
        ...
        # ReportAdditionalData Setup
        # Inside access bounds
        with pgtrigger.ignore("reporting.ReportAdditionalData:immutable_report_version"):
            report_additional_data_2010_submitted = make_recipe(
                'reporting.tests.utils.report_additional_data', report_version=t.report_version_2010_submitted
            )
        report_additional_data_2010_draft = make_recipe(
            'reporting.tests.utils.report_additional_data', report_version=t.report_version_2010_draft
        )
        # Outside access bounds
        report_additional_data_2013_draft = make_recipe(
            'reporting.tests.utils.report_additional_data', report_version=t.report_version_2013_draft
        )
        ...
        def select_function(cursor):
            ReportAdditionalData.objects.get(id=report_additional_data_2010_submitted.id)

        def forbidden_select_function(cursor):
            ReportAdditionalData.objects.get(id=report_additional_data_2013_draft.id)
        ...
        def forbidden_insert_function(cursor):
            cursor.execute(
                """
                    INSERT into "erc"."report_additional_data"(report_version_id)
                    values(%s)
                """,
                (report_additional_data_2013_draft.id,),
            )
        ...
        def update_function(cursor):
            cursor.execute(
                """
                    ...
                    SET capture_emissions = %s
                    WHERE id = %s
                """,
                ('true', report_additional_data_2010_draft.id),
            )
        ...
        def forbidden_update_function(cursor):
            cursor.execute(
                """
                    ...
                    WHERE id = %s
                """,
                ('true', report_additional_data_2013_draft.id),
            )
            return cursor.rowcount
        ...
        def delete_function(cursor):
            ...
            cursor.execute(
                """
                    DELETE from "erc"."report_additional_data"
                    WHERE id = %s
                """,
                (report_additional_data_2010_draft.id,),
            )
            return cursor.rowcount
        ...
        def forbidden_delete_function(cursor):
            ...
            cursor.execute(
                """
                    ...
                    WHERE id in (%s,%s)
                """,
                (report_additional_data_2010_submitted.id, report_additional_data_2013_draft.id),
            )
            return cursor.rowcount
