...
class ReportAttachmentRlsTest(BaseTestCase):
    ...
    def test_report_attachment_rls_industry_user(self):
        ...
        number_of_accessible_records = ReportAttachment.objects.filter(report_version=test.report_version).count()
        ...
        def select_function(cursor):
            ...
            assert ReportAttachment.objects.count() == number_of_accessible_records
        ...
        def insert_function(cursor):
            ...
            number_of_accessible_records_after_insert = number_of_accessible_records + 1

            assert ReportAttachment.objects.count() == number_of_accessible_records_after_insert
        ...
        def update_function(cursor):
            ...
            assert ReportAttachment.objects.get(id=test_report_attachment.id).attachment_name == "Updated_Attachment.pdf"
            ...
            cursor.execute(
                """
                    ...
                    WHERE "id" = %s
                """,
                ("Failed_Updated_Attachment.pdf", random_report_attachment.id),
            )
        ...
        def delete_function(cursor):
            ...
            assert (
                ReportAttachment.objects.filter(report_version=test.report_version).count()
                == number_of_accessible_records - number_of_deleted_report_attachments
            )
        ...
        test_policies_for_industry_user(ReportAttachment, test.approved_user_operator.user, select_function, insert_function, update_function, delete_function)
