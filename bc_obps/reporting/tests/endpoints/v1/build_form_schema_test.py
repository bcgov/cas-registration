...
class TestBuildFormSchema:
    ...
    def test_invalid_report_date_does_not_return_a_record(self):
        ...
        assert (
            ...
            == "No base schema found for activity_id 1 & source_type_id 1 & report_date 1800-01-01"
        )
    ...
    def test_no_base_schema_found(self):
        ...
        assert (
            response.json().get('message')
            == "No base schema found for activity_id 1 & source_type_id 9999 & report_date 2024-04-01"
        )
    ...
    def test_no_config_found(self):
        ...
        assert (
            response.json().get('message')
            == "No configuration found for activity_id 1 & source_type_id 1 & gas_type_id 1 & methodology_id 9999 & report_date 2024-04-01"
        )
    ...
    def test_gsc_co2_methodologies(self):
        ...
        assert response.status_code == 200
        assert json.loads(response.json())['schema']['properties']['methodology']['enum'] == [
