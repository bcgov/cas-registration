...
class TestComplianceSummaryService(TestCase):
    def test_raise_error_if_naics_code_unregulated(self):
        ## SETUP ##
        ...
        unregulated_naics_code = NaicsCode.objects.filter(is_regulated=False).first()
        Operation.objects.filter(pk=report_version_1.report.operation.id).update(naics_code_id=unregulated_naics_code)
        Report.objects.filter(pk=report_version_1.report_id).update(reporting_year_id=2024)

        with pytest.raises(
            ValidationError,
            match=f"NAICS code {unregulated_naics_code.naics_code} is not applicable to regulated operations. Cannot calculate compliance data.",
        ):
