...
class TestResolveComplianceParameters(unittest.TestCase):
    def test_apr_dec_prorates_correctly(self):
        ...
        # raw prorated value
        self.assertEqual(prorated_allocated, Decimal("10"))
    ...
    def test_jan_mar_prorates_correctly(self):
        ...
        production_for_limit, prorated_allocated, allocated_value = resolve_compliance_parameters(
            production_period="jan_mar", allocated_for_compliance=Decimal("100"), production_totals=production_totals
        )

        self.assertEqual(production_for_limit, Decimal("200"))
        # raw prorated value
        self.assertEqual(prorated_allocated, Decimal("20"))
        # rounded value used for product reporting
        self.assertEqual(allocated_value, Decimal("20.0000"))
    ...
    def test_jan_mar_with_zero_annual_returns_zero_prorated(self):
        ...
        # production_for_limit should be jan-mar value even if annual is zero
        self.assertEqual(production_for_limit, Decimal("75"))
        # prorated allocated should be zero to avoid division by zero
        self.assertEqual(prorated_allocated, Decimal("0"))
        self.assertEqual(allocated_value, Decimal("0.0000"))
