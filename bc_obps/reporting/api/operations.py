...
from reporting.schema.operation import (
    ReportingDashboardCurrentOperationOut,
    ReportingDashboardOperationFilterSchema,
    ReportingDashboardPastOperationOut,
)
...
@router.get(
    "/operations",
    response={200: List[ReportingDashboardCurrentOperationOut], custom_codes_4xx: Message},
    tags=DASHBOARD_TAGS,
...
def get_dashboard_operations_list(
    ...
    return ReportingDashboardService.get_operations_for_reporting_dashboard(
        user_guid, reporting_year, is_past_reports=False, sort_field=sort_field, sort_order=sort_order, filters=filters
    )


@router.get(
    "/past-operations",
    response={200: List[ReportingDashboardPastOperationOut], custom_codes_4xx: Message},
    tags=DASHBOARD_TAGS,
    description="""Returns a list of past reports for the current user.""",
    auth=authorize("approved_authorized_roles"),
)
@paginate(PageNumberPagination)
def get_past_reports_operations_list(
    request: HttpRequest,
    filters: ReportingDashboardOperationFilterSchema = Query(...),
    ...
    print(
        "API past reports",
        ReportingDashboardService.get_past_operations_for_reporting_dashboard(
            user_guid, reporting_year, sort_field=sort_field, sort_order=sort_order, filters=filters
        ),
    )
    return ReportingDashboardService.get_past_operations_for_reporting_dashboard(
        user_guid, reporting_year, sort_field=sort_field, sort_order=sort_order, filters=filters
    )
