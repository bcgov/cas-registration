...
class Rls:
    enable_rls = True
    schema = 'erc'
    table = ReportingTableNames.REPORT_NEW_ENTRANT_PRODUCTION
    role_grants_mapping = {
    ...
    using_statement = """
        report_new_entrant_id IN (
            SELECT rne.id
            FROM erc.report_new_entrant rne
            JOIN erc.report_version rv ON rne.report_version_id = rv.id
