...
class ComplianceInvoiceService:
    ...
    def generate_invoice_pdf(
        ...
        Args:
            compliance_report_version_id: ID of the compliance report compliance_report_version(for now we are mocking the data and not using the id)

        Returns:
            Tuple of (PDF data generator, filename, total_size_in_bytes)
        """
        context = ComplianceInvoiceService._prepare_invoice_context(compliance_report_version_id)
        ...
        return PDFGeneratorService.generate_pdf(
            template_name='invoice.html', context=context, filename=filename
        )
    ...
    def _prepare_invoice_context(
        ...
        try:
            operator_address: Address = operator.mailing_address
        ...
        try:
            compliance_obligation: ComplianceObligation = (
                ComplianceReportVersionService.get_obligation_by_compliance_report_version(
                    compliance_report_version_id
                )
            )
            ...
            msg = (
                f"No ComplianceObligation found for compliance_report_version ID "
                f"{compliance_report_version_id}."
            )
        ...
        # --------------------------------------------------------------------
        try:
            # a) From compliance_obligation → ComplianceReportVersion
        ...
        try:
            # b) From compliance_report_version → ComplianceReport
        ...
        try:
            # c) From compliance_report → CompliancePeriod
            compliance_period = compliance_report.compliance_period
            if not compliance_period:
                raise ObjectDoesNotExist
        except ObjectDoesNotExist:
            msg = (
                f"ComplianceReport (ID {getattr(compliance_report, 'pk', 'unknown')}) "
                "has no CompliancePeriod."
            )
            errors["missing_compliance_period"] = msg
            return {"errors": errors}

        try:
            # d) From compliance_period → ReportingYear
            ...
            msg = (
                f"CompliancePeriod (ID {getattr(compliance_period, 'pk', 'unknown')}) "
                "has no reporting_year."
            )
            errors["missing_reporting_year"] = msg
            return {"errors": errors}

        # e) Find the ComplianceChargeRate for that reporting_year
        try:
            charge_rate_obj = ComplianceChargeRate.objects.get(
                reporting_year=reporting_year_obj
            )
            charge_rate_decimal: Decimal = charge_rate_obj.rate
        except ObjectDoesNotExist:
            msg = (
                f"No ComplianceChargeRate found for ReportingYear "
                f"{context_reporting_year}."
            )
        ...
        try:
            ...
            equivalent_amount_decimal = raw_equivalent.quantize(
        ...
        # --------------------------------------------------------------------
        try:
            invoice_number = "OBI+six-digits"  # replace with real ID if you have one
            ...
            operator_address_line2 = (
                f"{operator_address.municipality}, "
                f"{operator_address.province}, "
                f"{operator_address.postal_code}"
            )

            operation_name = operation.name
            if hasattr(operation, "physical_address") and operation.physical_address:
                op_addr = operation.physical_address
                operation_address_line1 = op_addr.street_address
                operation_address_line2 = (
                    f"{op_addr.municipality}, "
                    f"{op_addr.province}, "
                    f"{op_addr.postal_code}"
                )
            else:
                operation_address_line1 = ""
                operation_address_line2 = ""
        except Exception as exc:
        ...
        try:
            ...
            total_amount = (
                f"${total_amount_numeric.quantize(Decimal('0.01'), rounding=ROUND_HALF_UP):,}"
            )
        except Exception as exc:
