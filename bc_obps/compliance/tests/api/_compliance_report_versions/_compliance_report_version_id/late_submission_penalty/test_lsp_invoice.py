...
class TestGenerateComplianceReportVersionLateSubmissionPenaltyInvoice(CommonTestSetup):
    @patch(
        "compliance.service.elicensing_invoice_service.ElicensingInvoiceService.generate_late_submission_penalty_invoice_pdf"
    )
    ...
    def test_get_invoice_success(
        self,
        mock_create_pdf_response,
        mock_generate_late_submission_penalty_invoice_pdf,
    ):
        # Arrange
        pdf_bytes = b"%PDF content"
        streaming_content = iter([pdf_bytes])

        mock_generate_late_submission_penalty_invoice_pdf.return_value = (
            iter([pdf_bytes]),
            "invoice.pdf",
            len(pdf_bytes),
        )
        ...
        response = TestUtils.mock_get_with_auth_role(
            ...
            "industry_user",
            custom_reverse_lazy(
                "generate_compliance_report_version_late_submission_penalty_invoice",
        ...
        mock_generate_late_submission_penalty_invoice_pdf.assert_called_once_with(compliance_report_version.id)
        mock_create_pdf_response.assert_called_once_with((ANY, "invoice.pdf", 12))

    @patch(
        "compliance.service.elicensing_invoice_service.ElicensingInvoiceService.generate_late_submission_penalty_invoice_pdf"
    )
    def test_get_invoice_error_from_service(self, mock_generate_late_submission_penalty_invoice_pdf):
