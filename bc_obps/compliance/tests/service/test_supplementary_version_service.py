...
class TestDecreasedCreditHandler(BaseSupplementaryVersionServiceTest):
    ...
    def test_can_handle_returns_false_when_no_previous_credit(self):
        with pgtrigger.ignore('reporting.ReportComplianceSummary:immutable_report_version'):
            prev = baker.make_recipe('reporting.tests.utils.report_compliance_summary',
                                    credited_emissions=Decimal('600'), report_version=self.report_version_1)
            new  = baker.make_recipe('reporting.tests.utils.report_compliance_summary',
                                    credited_emissions=Decimal('500'), report_version=self.report_version_2)
        baker.make_recipe('compliance.tests.utils.compliance_report_version',
                        report_compliance_summary=prev, is_supplementary=False)
    ...
    def test_handle_approved_prior_credit_marks_manual_handling_and_keeps_approved_credit(self):
        ...
        prev_crv = baker.make_recipe('compliance.tests.utils.compliance_report_version',
                                    report_compliance_summary=prev, is_supplementary=False)
        approved = baker.make_recipe('compliance.tests.utils.compliance_earned_credit',
