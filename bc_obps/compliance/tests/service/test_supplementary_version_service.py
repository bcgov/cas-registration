...
class TestSupplementaryVersionService(BaseSupplementaryVersionServiceTest):
    def test_handle_supplementary_version_pending_manual_handling_success(
        ...
        # ManualHandler.can_handle(previous_summary) will be true when PENDING_MANUAL_HANDLING
        baker.make_recipe(
...
class TestManualHandler(BaseSupplementaryVersionServiceTest):
    def test_can_handle_report_that_previously_required_pending_manual_handling(self):
        # Arrange
        with pgtrigger.ignore('reporting.ReportComplianceSummary:immutable_report_version'):
        ...
        # Previous CRV that *does* have a pending manual-handling record
    ...
    def test_cannot_handle_report_that_previously_required_resolved_manual_handling(self):
        # Arrange
        with pgtrigger.ignore('reporting.ReportComplianceSummary:immutable_report_version'):
            previous_summary = baker.make_recipe(
                'reporting.tests.utils.report_compliance_summary',
        ...
        # Previous CRV that has a manual-handling record, but it has already been resolved
        previous_crv = baker.make_recipe(
            'compliance.tests.utils.compliance_report_version',
            compliance_report=compliance_report,
            report_compliance_summary=previous_summary,
