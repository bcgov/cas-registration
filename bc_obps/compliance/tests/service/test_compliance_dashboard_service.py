...
class TestComplianceDashboardService:
    ...
    def test_get_compliance_report_versions_annotates_status_correctly(self, mock_current_reporting_year):
        ...
        def create_compliance_report_version(
            ...
            earned_credit_issuance_status=None,
            earned_credit_kwargs=None,
        ):
            operation = make_recipe(
                'registration.tests.utils.operation',
                operator=user_operator.operator,
            ...
            if earned_credit_issuance_status:
                kwargs = earned_credit_kwargs or {}
                make_recipe(
                    'compliance.tests.utils.compliance_earned_credit',
                    issuance_status=earned_credit_issuance_status,
                    compliance_report_version_id=compliance_report_version.id,
                    **kwargs,
                )
            return compliance_report_version

        crv_manual_handling = create_compliance_report_version(
            status=ComplianceReportVersion.ComplianceStatus.OBLIGATION_NOT_MET,
            requires_manual_handling=True,
        )
        crv_not_met = create_compliance_report_version(
            status=ComplianceReportVersion.ComplianceStatus.OBLIGATION_NOT_MET,
        )
        crv_met = create_compliance_report_version(
            status=ComplianceReportVersion.ComplianceStatus.OBLIGATION_FULLY_MET,
        )
        crv_pending_invoice = create_compliance_report_version(
            status=ComplianceReportVersion.ComplianceStatus.OBLIGATION_PENDING_INVOICE_CREATION,
        )
        crv_not_requested = create_compliance_report_version(
            status=ComplianceReportVersion.ComplianceStatus.EARNED_CREDITS,
            earned_credit_issuance_status=ComplianceEarnedCredit.IssuanceStatus.CREDITS_NOT_ISSUED,
        )
        crv_issuance_requested = create_compliance_report_version(
            status=ComplianceReportVersion.ComplianceStatus.EARNED_CREDITS,
            earned_credit_issuance_status=ComplianceEarnedCredit.IssuanceStatus.ISSUANCE_REQUESTED,
            earned_credit_kwargs={
                "bccr_trading_name": "test bccr trading name",
                "bccr_holding_account_id": "12346",
            },
        )
        crv_approved = create_compliance_report_version(
            status=ComplianceReportVersion.ComplianceStatus.EARNED_CREDITS,
            earned_credit_issuance_status=ComplianceEarnedCredit.IssuanceStatus.APPROVED,
            earned_credit_kwargs={
                "bccr_trading_name": "test bccr trading name 2",
                "bccr_holding_account_id": "12347",
            },
        )
        crv_declined = create_compliance_report_version(
            status=ComplianceReportVersion.ComplianceStatus.EARNED_CREDITS,
            earned_credit_issuance_status=ComplianceEarnedCredit.IssuanceStatus.DECLINED,
            earned_credit_kwargs={
                "bccr_trading_name": "test bccr trading name 3",
                "bccr_holding_account_id": "12348",
            },
        )
        crv_changes_required = create_compliance_report_version(
            status=ComplianceReportVersion.ComplianceStatus.EARNED_CREDITS,
            earned_credit_issuance_status=ComplianceEarnedCredit.IssuanceStatus.CHANGES_REQUIRED,
            earned_credit_kwargs={
                "bccr_trading_name": "test bccr trading name 4",
                "bccr_holding_account_id": "12349",
            },
        )
        crv_earned_credits = create_compliance_report_version(
            status=ComplianceReportVersion.ComplianceStatus.EARNED_CREDITS,
        )
        ...
        # Assert
        expected_results = [
            (crv_manual_handling.id, "Supplementary report - action required"),
            (crv_not_met.id, "Obligation - not met"),
            (crv_met.id, "Obligation - met"),
            (crv_pending_invoice.id, "Obligation - pending invoice creation"),
            (crv_not_requested.id, "Earned credits - not requested"),
            (crv_issuance_requested.id, "Earned credits - issuance requested"),
            (crv_approved.id, "Earned credits - approved"),
            (crv_declined.id, "Earned credits - declined"),
            (crv_changes_required.id, "Earned credits - changes required"),
            (crv_earned_credits.id, "Earned credits"),
        ]

        for idx, (expected_id, expected_status) in enumerate(expected_results):
            assert result[idx].id == expected_id
            assert result[idx].display_status == expected_status
