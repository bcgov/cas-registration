...
class TestTaskService(TestCase):
    ...
    def test_get_due_tasks_different_statuses_scheduled_task(self):
        # Test PENDING status
        self.scheduled_task.status = ScheduledTask.TaskStatus.PENDING
        self.scheduled_task.save()
        due_tasks = TaskService.get_due_tasks()
        self.assertIn(self.scheduled_task, due_tasks)

        # Test FAILED status
        self.scheduled_task.status = ScheduledTask.TaskStatus.FAILED
        self.scheduled_task.save()
        due_tasks = TaskService.get_due_tasks()
        self.assertIn(self.scheduled_task, due_tasks)

        # Test COMPLETED status
        self.scheduled_task.status = ScheduledTask.TaskStatus.COMPLETED
        self.scheduled_task.save()
        due_tasks = TaskService.get_due_tasks()
        self.assertIn(self.scheduled_task, due_tasks)

        # Test INACTIVE status (should not be included)
        self.scheduled_task.status = ScheduledTask.TaskStatus.INACTIVE
        self.scheduled_task.save()
        due_tasks = TaskService.get_due_tasks()
    ...
    def test_get_due_tasks_different_statuses_retry_task(self):
        # Test PENDING status
        self.retry_task.status = RetryTask.TaskStatus.PENDING
        self.retry_task.save()
        due_tasks = TaskService.get_due_tasks()
        self.assertIn(self.retry_task, due_tasks)

        # Test FAILED status
        self.retry_task.status = RetryTask.TaskStatus.FAILED
        self.retry_task.save()
        due_tasks = TaskService.get_due_tasks()
        self.assertIn(self.retry_task, due_tasks)

        # Test COMPLETED status (should not be included)
        self.retry_task.status = RetryTask.TaskStatus.COMPLETED
        self.retry_task.save()
        due_tasks = TaskService.get_due_tasks()
