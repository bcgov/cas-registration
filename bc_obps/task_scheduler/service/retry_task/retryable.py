...
def create_retry_task(
    ...
    existing_task = RetryTask.objects.filter(
        ...
        kwargs=kwargs,
        tag=tag,
        task_status__in=[RetryTask.Status.PENDING, RetryTask.Status.RUNNING, RetryTask.Status.FAILED],
    ).first()

    if existing_task:
        if existing_task.task_status == RetryTask.Status.FAILED and existing_task.can_retry:
