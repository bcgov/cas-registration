...
def get_first_project() -> str:
    ...
    result = subprocess.run(
        ['oc', 'get', 'projects', '-o', 'name'], stdout=subprocess.PIPE, check=True, text=True
    )  # nosec
...
def list_pods(project: str) -> None:
    ...
    result = subprocess.run(
        [*oc_n_project(project), 'get', 'pods'], stdout=subprocess.PIPE, check=True, text=True
    )  # nosec
    print(result.stdout)
...
def list_projects() -> None:
    ...
    result = subprocess.run(['oc', 'get', 'projects'], stdout=subprocess.PIPE, check=True, text=True)  # nosec
    print(result.stdout)
...
def list_cluster_members(project: str, pod: str) -> None:
    ...
    result = subprocess.run(
        [*oc_rsh(project, pod), 'patronictl', 'list'], stdout=subprocess.PIPE, check=True, text=True  # nosec
    )
...
def list_databases(project: str, pod: str) -> None:
    ...
    result = subprocess.run(
        [*oc_rsh(project, pod), 'psql'], input='\\l', stdout=subprocess.PIPE, check=True, text=True, timeout=20  # nosec
    )
    print(result.stdout)
...
def list_schemas(project: str, pod: str, database: str) -> None:
    ...
    result = subprocess.run(
        [*oc_rsh(project, pod), 'psql', '-d', database],
        input='\\dn',
...
def create_lock_file(project: str, pod: str, database: str) -> None:
    ...
    try:
        lock_file = '/tmp/{}.lock'.format(database)  # nosec
        print('creating lock file: {}'.format(lock_file))
        subprocess.run(
            [*oc_rsh(project, pod), 'mkdir', lock_file], stdout=subprocess.PIPE, check=True, text=True
        )  # nosec
...
def delete_lock_file(project: str, pod: str, database: str) -> None:
    ...
    result = subprocess.run(
        [*oc_rsh(project, pod), 'rmdir', lock_file], stdout=subprocess.PIPE, check=True, text=True
    )  # nosec
    print(result.stdout)
...
def dump_database(project: str, pod: str, database: str, schema: str) -> List[str]:
    ...
    for table in tables:
        csv_file = '/tmp/{}.csv'.format(table)  # nosec
        files.append(csv_file)
        sql_cmd = (
            'psql {database} -c "copy (SELECT * FROM {schema}.{table}) to '  # nosec
...
def copy_files_to_local(project: str, pod: str, files: List[str]) -> None:
    ...
    for filename in files:
        ...
        result = subprocess.run(
            [*oc_n_project(project), 'cp', '{}:{}.gz'.format(pod, filename), './{}.gz'.format(filename)],  # nosec
            stdout=subprocess.PIPE,
            check=True,
            text=True,
        )
...
def unzip_locally(files: List[str]) -> None:
    ...
    for filename in files:
        ...
        result = subprocess.run(
            ['gunzip', '.{}.gz'.format(filename)], stdout=subprocess.PIPE, check=True, text=True
        )  # nosec
        print(result.stdout)
