...
@router.put(
    "/select-operator/user-operator/page-1/{int:user_operator_id}",
    response={200: SelectOperatorIn, codes_4xx: Message},
)
def create_user_operator_page_1_request(request, user_operator_id: int, payload: UserOperatorIn):
    try:
        user_operator = get_object_or_404(UserOperator, id=user_operator_id)
        user: User = user_operator.user
        operator: Operator = user_operator.operator
...
@router.put(
    "/select-operator/user-operator/submit/{int:user_operator_id}",
    response={200: SelectOperatorIn, codes_4xx: Message},
)
def create_user_operator_submit_request(request, user_operator_id: int, payload: UserOperatorIn):
    try:
        user_operator = get_object_or_404(UserOperator, id=user_operator_id)
        operator: Operator = user_operator.operator
        payload_dict = payload.dict()
