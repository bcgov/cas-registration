...
@router.get(
    "/user-operators/current/has_registered_regulated_operation",
    response={200: Dict[str, bool], custom_codes_4xx: Message},
    tags=USER_OPERATOR_TAGS,
    description="""
        Checks if a current user's operator has a registered and regulated operation.
        If it does, it returns 'True'; otherwise, it returns 'False'.
    """,
    auth=authorize("industry_user"),
)
def get_current_user_operator_has_registered_regulated_operation(
    request: HttpRequest,
    ...
    try:
        ...
        if not operator.user_has_access(current_user_guid):
            return 200, {"has_registered_regulated_operation": False}
        # Use the service to check if the operator has a registered and regulated operation
        has_registered_regulated_operation = (
            OperationDataAccessService.check_current_users_registered_regulated_operation(operator.id)
        )
        return 200, {"has_registered_regulated_operation": has_registered_regulated_operation}
