...
class TestBcGreenhouseGasIdModel(BaseTestCase):
    @classmethod
    def setUpTestData(cls):
        cls.test_object = BcGreenhouseGasId.objects.create(id='23279909999', issued_at=timezone.now(), comments='test')
        cls.field_data = [
    ...
    def test_id_should_be_unique(self):
        ...
        with self.assertRaises(ValidationError, msg='Bc obps regulated operation with this Id already exists.'):
            BcGreenhouseGasId.objects.create(id='23279909999', comments='test')  # already created in setUpTestData
