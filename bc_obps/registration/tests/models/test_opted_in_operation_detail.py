...
class TestOptedInOperationDetailRls(BaseTestCase):
    def test_opted_in_operation_detail_rls_industry_user(self):
        ...
        operation = baker.make_recipe('registration.tests.utils.operation', operator=approved_user_operator.operator, opted_in_operation=opted_in_operation_detail)


        random_approved_user_operator = baker.make_recipe('registration.tests.utils.approved_user_operator')
        random_opted_in_operation_detail = baker.make_recipe(
            'registration.tests.utils.opted_in_operation_detail'
        )
        random_operation = baker.make_recipe('registration.tests.utils.operation', operator=random_approved_user_operator.operator, opted_in_operation=random_opted_in_operation_detail)
        ...
        def update_function(cursor):
            ...
            assert OptedInOperationDetail.objects.filter(meets_electricity_import_operation_criteria=True).count() == 1  # only affected 1 
    ...
    # brianna fix
        ...
        test_policies_for_industry_user(
            OptedInOperationDetail, approved_user_operator.user, select_function=select_function, insert_function=insert_function, 
    ...
    def test_opted_in_operation_detail_rls_cas_users(self):
        ...
        test_policies_for_cas_roles(OptedInOperationDetail, select_function=select_function, )
