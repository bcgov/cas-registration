...
class TestAddressRls(BaseTestCase):
    ...
    def test_address_rls_industry_user_success(self):
        ...
        def select_function(cursor):
            ...
            assert cursor.fetchone()[0] == 2  # Industry user should see the addresses associated with their operator and contact, but not the random contact's
        ...
        def delete_function(cursor):
            ...
            breakpoint()
            assert cursor.rowcount == 1  # User should be able to delete their own address brianna and the operator address??
    ...
    def test_address_rls_industry_user_fail(self):
        approved_user_operator = baker.make_recipe('registration.tests.utils.approved_user_operator') # This creates an address record 
        random_address = baker.make_recipe(
            'registration.tests.utils.address')
        random_contact = baker.make_recipe(
            'registration.tests.utils.contact', operator=baker.make_recipe('registration.tests.utils.operator'), address=random_address)
