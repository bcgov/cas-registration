...
class MultipleOperatorIn(Schema):
    ...
    mo_bc_corporate_registry_number: str = Field(pattern=r"^[A-Za-z]{1,3}\d{7}$")
...
class MultipleOperatorOut(Schema):
    ...
    mo_bc_corporate_registry_number: str = Field(
        ..., alias="bc_corporate_registry_number", pattern=r"^[A-Za-z]{1,3}\d{7}$"
    )
    mo_business_structure: str = Field(..., alias="business_structure")
    mo_website: Optional[str] = Field(None, alias="website")
    mo_percentage_ownership: Optional[int] = Field(None, alias="percentage_ownership")
