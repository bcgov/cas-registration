...
def count_stats(Contact) -> Dict[str, int]:
    ...
    return {
        ...
        .distinct()
        .count(),
        'total_unique_emails_and_operators': Contact.objects.values('email', 'operator').distinct().count(),
...
def migrate_remove_duplicate_contacts(apps, schema_monitor):
    ...
    if before_stats['total_unique_contacts'] != before_stats['total_unique_emails_and_operators']:
    ...
    assert after_stats['total_contacts'] == after_stats['total_unique_emails_and_operators']
