...
def migrate_assign_regulated_purpose(apps, schema_monitor):
    ...
    for operation in Operation.objects.filter(bc_obps_regulated_operation__isnull=False):
        if operation.opt_in:
            ...
            operation.opted_in_operation = OptedInOperationDetail.objects.create(
                created_by_id=operation.created_by.user_guid
            )
        else:
    ...
    assert after_stats.get('has_boro_id') == after_stats.get('has_regulated_purpose') + after_stats.get(
        'has_opt_in_purpose'
    )
