...
class ApplicationAccessService:
    ...
    def does_user_belong_to_operator(operator_id: int, user_guid: UUID):
        ...
        users_business_bceid = UserDataAccessService.get_user_by_guid(user_guid)
    ...
    def is_user_eligible_to_request_admin_access(user_guid: UUID, operator_id: UUID):
        ...
        approved_admins = UserOperatorDataAccessService.get_approved_admins(operator_id)

        # brianna psuedo
        if user_guid in approved_admins:
            raise ("You are already an admin for this Operator!")

        if len(approved_admins) > 1:
            raise ("This Operator already has an admin user!")
        ...
        pending_admins = UserOperatorDataAccessService.get_pending_admins(operator_id)

        # brianna psuedo
        if user_guid in pending_admins:
            raise ("You already have a pending request for this Operator!")
    ...
    def request_access(operator_id: UUID, user_guid: UUID):
        if ApplicationAccessService.does_user_belong_to_operator(operator_id, user_guid):
            ...
            user_operator = UserOperatorDataAccessService.get_or_create_user_operator()
    ...
    def get_allowed_user_operator(user_guid: UUID, user_operator_id: UUID):
        ...
        if user.is_industry_user():
            ...
            if user_operator.user.user_guid != user_guid:
                ...
                raise ("Your user is not associated with this operator.")
