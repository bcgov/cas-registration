...
class ComplianceObligationService:
    ...
    @classmethod
    def _get_obligation_id(cls, report_version: ReportVersion) -> Optional[str]:
        """
        ...
        Returns:
            Optional[str]: The formatted obligation ID, or None if the operation is not approved
        """
        # Get BORO ID if available (format: YY-OOOO)
        if report_version.report.operation.bc_obps_regulated_operation:
            # Use the BORO ID directly (already in YY-OOOO format)
            operation_part = report_version.report.operation.bc_obps_regulated_operation.id
        else:
