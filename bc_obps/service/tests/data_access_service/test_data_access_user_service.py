...
class TestDataAccessUserService:
    ...
    def test_get_by_guid():
        cas_admin = baker.make_recipe('cas_admin')
    ...
    def test_get_by_guid_skip_archived():
        ...
        archived_user = baker.make_recipe('cas_director',archived_at=timezone.now(), archived_by=cas_admin)
        
        assert UserDataAccessService.get_by_guid(archived_user.user_guid).count() == 0
    ...
    def test_get_internal_users_including_archived():
        ...
        cas_admin = baker.make_recipe('cas_admin')
        # archived internal user
        baker.make_recipe('cas_director',archived_at=timezone.now(), archived_by=cas_admin)
        # external user - should not be included in results
        baker.make_recipe('industry_operator_user')
        # archived external user - should not be included in results
        baker.make_recipe('industry_operator_user',archived_at=timezone.now(), archived_by=cas_admin)
