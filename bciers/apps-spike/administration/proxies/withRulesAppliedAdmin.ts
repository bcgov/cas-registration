...
const handleIndustryUserRoutes = async (request: NextRequest, token: any) => {
  ...
  if (pathname.endsWith("select-operator")) {
    try {
      ...
      // 🧩 If there is no userOperator, proceed to the next proxy
      if (!userOperator) {
        return null; // No redirect required, continue to the next proxy
      }
      ...
      // Proceed to next proxy in case of error
      return null;
    }
  }
