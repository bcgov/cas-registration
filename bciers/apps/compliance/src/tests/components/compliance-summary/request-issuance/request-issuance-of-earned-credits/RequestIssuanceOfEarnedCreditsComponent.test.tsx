...
describe("RequestIssuanceOfEarnedCreditsComponent", () => {
  ...
  it("clears errors and does not display a generic error message when account validation succeeds", async () => {
    // Mock the successful API response
    (getBccrAccountDetails as ReturnType<typeof vi.fn>).mockResolvedValueOnce({
      bccr_trading_name: "Bravo Technologies 2026", // Match whatever mock name you use in this file
    });

    render(
      <RequestIssuanceOfEarnedCreditsComponent
        complianceReportVersionId={123} // Use your TEST_COMPLIANCE_REPORT_VERSION_ID constant if you have one
        data={{}} // Pass whatever mock initial data you use in this file
      />,
    );

    const accountInput = screen.getByLabelText(/BCCR Holding Account ID/i);
    fireEvent.change(accountInput, { target: { value: "103200000029267" } });

    await waitFor(() => {
      expect(screen.getByText("Bravo Technologies 2026")).toBeVisible();
      expect(
