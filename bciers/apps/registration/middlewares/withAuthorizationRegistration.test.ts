...
describe("withAuthorizationRegistration middleware", () => {
  ...
  it("calls NextMiddleware if the user is authenticated and the path is in the authenticated allow list", async () => {
    ...
    const nextUrl = new NextURL(`${domain}/registration/profile`);
  ...
  it("redirects industry users to the registration page if their operator is not pending or approved", async () => {
    ...
    expect(NextResponse.redirect).toHaveBeenCalledOnce();
    expect(NextResponse.redirect).toHaveBeenCalledWith(
      new URL("/registration", domain),
    );
    expect(result?.status).toBe(307);
  });

  it("redirects industry users to the registration page if they don't have an operator", async () => {
    getToken.mockResolvedValue(mockIndustryUserToken);
    const nextUrl = new NextURL(`${domain}/registration/operations`);
  ...
  it("does not redirect industry users to the registration page if their operator is approved", async () => {
  ...
  it("does not redirect industry users to the registration page if their operator is pending", async () => {
    getToken.mockResolvedValue({
      ...mockIndustryUserToken,
      user_guid: "feb4d26d-45e1-437a-b53f-b25e617c388f",
    });
  ...
  it("builds the correct URL for industry users", async () => {
    getToken.mockResolvedValue(mockIndustryUserToken);
    const nextUrl = new NextURL(`${domain}/registration/operators`);
