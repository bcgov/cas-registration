...
> = {
  ...
  report_data_out_of_bounds_by_fuel_type: createValidationUIConfig({
    ...
    formatMessage: ({ error }) => {
      ...
      return `Unusual value detected for ${ctx?.facility_name ?? "facility"} ${ctx?.activity_name ?? "activity"}.
Expected ${ctx?.fuel_type_name ?? "fuel"} value to be within the allowed range,
