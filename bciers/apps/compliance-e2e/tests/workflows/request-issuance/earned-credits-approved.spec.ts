...
test.describe("Test earned credits approve request issuance flow", () => {
  test("Industry user submits request issuance → analyst sets as ready for approval → director approves", async ({
    browser,
    baseURL,
    request,
  }) => {
    //Wrapper that closes over { browser, baseURL } to call newContextForRole for role switching
    ...
    // 🔌 Set up stub to mock the call to getBccrAccountDetails
    await industryEarnedCredits.attachBccrAccountValidationStub(
      "Mock Trading Name Inc.",
    );
    // 🔌 Attach the stub to by-pass call to API so PUT /earned-credits is routed to /e2e-integration-stub
    await industryEarnedCredits.attachRequestIssuanceStub(
      request,
      baseURL as string,
    );
