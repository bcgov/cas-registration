...
async function assertPenaltyCalculatorLoaded(page: any) {
	...
	await expect(page.getByText("GGEAPAR interest:", { exact: true })).toBeVisible();
	await expect(
		page.getByLabel(/2\. Select final day of penalty accrual/i),
	).toBeVisible();
	await expect(page.getByText("Penalty summary", { exact: true })).toBeVisible();
...
async function ensureFinalDayOfAccrualInputValue(page: any) {
	...
	const finalDayValue = await finalDayOfAccrualInput.inputValue();
...
function parseOutstandingBalanceTco2e(value: string): number {
	...
	if (!match?.[1]) {
		throw new Error(`Could not parse outstanding balance tCO2e from "${value}"`);
	}
...
async function getOperationRowOutstandingBalances(page: any) {
	...
	for (let i = 0; i < rowCount; i++) {
		...
		rowData.push({
			...
			hasActionLink: (await row.getByRole("link", { name: OBLIGATION_ACTION_LINK }).count()) > 0,
		});
	}
...
async function seedSupplementaryReportWithLargerObligationForGgeapar() {
	const industryPage = await openNewBrowserContextAs(UserRole.INDUSTRY_USER_ADMIN);
	...
	try {
		...
		await reports.route();
		const firstSupplementaryCrvId = await reports.supplementaryReportObligationDecrease(industryContext.request, {
		...
		const baselineActionableRow = baselineRows.find(
			(row) => row.hasActionLink && /obligation\s*-\s*not\s*met/i.test(row.status),
		);
		expect(
			baselineActionableRow,
			"Expected an actionable Obligation - not met row after first supplementary submission",
		).toBeDefined();
		const baselineOutstandingBalance = baselineActionableRow!.outstandingBalance;
		...
		await reports.route();
		const secondSupplementaryCrvId = await reports.supplementaryReportObligationDecrease(industryContext.request, {
			annualProduction: 1,
			productIndex: 1,
			reviewChangesReason:
		...
		await summaries.route();
		await expect(async () => {
			const postSupplementaryRows = await getOperationRowOutstandingBalances(
				industryPage,
			);
			const pendingInvoiceRows = postSupplementaryRows.filter((row) =>
				/pending\s*invoice\s*creation/i.test(row.status),
			);
			expect(pendingInvoiceRows.length).toBeGreaterThan(0);
			...
			expect(
				highestPendingOutstandingBalance,
				`Expected supplementary obligation outstanding balance to increase beyond baseline ${baselineOutstandingBalance}`,
			).toBeGreaterThan(baselineOutstandingBalance);
		}).toPass({ timeout: 30_000 });
	} finally {
...
analystTest.describe("Internal penalty calculator from compliance summaries", () => {
	analystTest.describe.configure({ timeout: 180_000 });

	analystTest.beforeAll(async () => {
	...
	analystTest(
		"internal user can open and use automatic overdue penalty calculator from manage obligation",
		async ({ page, happoScreenshot }: any) => {
	...
	analystTest(
		"internal user can open and use GGEAPAR penalty calculator from manage obligation",
		async ({ page, happoScreenshot }: any) => {
